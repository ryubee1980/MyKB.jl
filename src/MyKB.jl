...
In each function, the optional variable ``ave_range`` is introduced so that the radial distribution``g`` approaches 1 as ``r`` goes to infinity (``g`` is averaged in the range of end-ave_range:end, i.e., g_tail_ave=sum(g[end-ave_range:end])/(ave_range+1), and is normalized as  ``g/g_tail_ave`` for all ``r``).
"""
...
in_RDF(file)
"""
function in_RDF(file;ave_range=0,ave_end=0)
    fn=open(file,"r")
    ...
    if ave_end==0 && 0<ave_range<length(g)
        temp= sum(g[end-ave_range:end])/(ave_range+1)
        @. g/=temp
    elseif 0<ave_end<length(g) && 0<ave_range<ave_end
        temp= sum(g[ave_end-ave_range:ave_end])/(ave_range+1)
        @. g/=temp
    end
    close(fn)
    hcat(r,g)
...
Plot RDF from data file. By default, the ``g``-range is not specified, while ``r``-range is specified as ``rmin=0.0`` and ``rmax=3.0``.
    ...
    plot_RDF(file;ave_range=0,rmin=0.0,rmax=3.0,gmin=0,gmax=3.0)
"""
function plot_RDF(file;ave_range=0,ave_end=0,rmin=0.0,rmax=3.0,gmin=-1.0,gmax=-2.0)
    g=in_RDF(file,ave_range=ave_range,ave_end=ave_end)
    if(gmin>gmax)
        p=plot(g[:,1],g[:,2],xlabel="\$r\$",ylabel="\$g(r)\$", xlim=(rmin,rmax))
    else
        p=plot(g[:,1],g[:,2],xlabel="\$r\$",ylabel="\$g(r)\$", xlim=(rmin,rmax),ylim=(gmin,gmax))
    end
    p
end
...
Compute ``G(R)`` from a data file of RDF generated by GROMACS.
    ...
    comp_GR(file;ave_range=0)
"""
function comp_GR(file;ave_range=0,ave_end)
    g=in_RDF(file,ave_range=ave_range,ave_end=ave_end)
    h=hr(g)
    #dr=h[2,1]-h[1,1]
    RRmax=h[end,1]
...
Plot ``1/R`` vs ``G(R)`` in the range ``xmin < 1/R < xmax``.
    ...
    plot_GR(file;ave_range=0,xmin=0,xmax=4)
"""
function plot_GR(file;ave_range=0,ave_end=0,xmin=0,xmax=4)
    GR=comp_GR(file;ave_range=ave_range,ave_end=ave_end)
...
Plot ``R`` vs ``R*G(R)`` in the range ``xmin < R < xmax``.
    ...
    plot_RGR(file;ave_range=0,xmin=0,xmax=3)
"""
function plot_RGR(file;ave_range=0,ave_end=0,xmin=0,xmax=3)
    GR=comp_GR(file;ave_range=ave_range,ave_end=ave_end)
...
Perform fitting ``1/R`` vs ``G(R)`` by a linear function ``w[1]+(1/R)*w[2]`` in the range ``recRmin < 1/R < recRmax``. The KB integral in the inifinite volume ``G(∞ )`` is given by the extrapolation, i.e.,  ``w[1]``.
    ...
    fit_GR(file,recRmin,recRmax;ave_range=0)
"""
function fit_GR(file,recRmin,recRmax;ave_range=0,ave_end=0)
    GR=comp_GR(file,ave_range=ave_range,ave_end=ave_end)
...
Perform fitting ``R`` vs ``R*G(R)`` by a linear function ``w[1]+R*w[2]`` in the range ``Rmin < R < Rmax``. The KB integral in the inifinite volume ``G(∞ )`` is given by the extrapolation, i.e.,  ``w[2]``.
    ...
    fit_GR(file,recRmin,recRmax;ave_range=0)
"""
function fit_RGR(file,Rmin,Rmax;ave_range=0,ave_end=0)
    GR=comp_GR(file,ave_range=ave_range,ave_end=ave_end)
    x=Array{Float64}(undef,0)
    y=similar(x)
...
Evaluate KB integral using fit_GR.
    ...
    eval_KB(file,recRmin,recRmax;ave_range=0)
"""
function eval_KB(file,recRmin,recRmax;ave_range=0,ave_end=0)
    w=fit_GR(file,recRmin,recRmax,ave_range=ave_range,ave_end=ave_end)
    println("G(∞ )=", w[1], " /nm^3")
    w[1]
end
...
Evaluate KB integral using fit_RGR.
    ...
    eval_KB2(file,Rmin,Rmax;ave_range=0)
"""
function eval_KB2(file,Rmin,Rmax;ave_range=0,ave_end=0)
    w=fit_RGR(file,Rmin,Rmax,ave_range=ave_range,ave_end=ave_end)
    println("G(∞ )=", w[2], " /nm^3")
    w[2]
end
...
Plot ``1/R`` vs ``G(R)`` together with the fitting line in the range ``xmin < 1/R < xmax``.
    ...
    plot_GR_fit(file,recRmin,recRmax;ave_range=0,xmin=0,xmax=4,show_range=1)
"""
function plot_GR_fit(file,recRmin,recRmax;ave_range=0,ave_end=0,xmin=0,xmax=4,show_range=1)
    GR=comp_GR(file;ave_range=ave_range,ave_end=ave_end)
...
Plot ``R`` vs ``R*G(R)`` together with the fitting line in the range ``xmin < R < max``.
    ...
    plot_RGR_fit(file,Rmin,Rmax;ave_range=0,xmin=0,xmax=3,show_range=1)
"""
function plot_RGR_fit(file,Rmin,Rmax;ave_range=0,ave_end=0,xmin=0,xmax=3,show_range=1)
    GR=comp_GR(file;ave_range=ave_range,ave_end=ave_end)
    ...
    w=fit_RGR(file,Rmin,Rmax,ave_range=ave_range,ave_end=ave_end)
    x=Rmin:0.1:Rmax+0.5
    plot!(p,x,(@. w[1]+w[2]*x),ls=:dot,label="\$A+\$$(w[2])\$R\$")
    if(show_range==1)
