...
in_RDF(file)
"""
function in_RDF(file;ave_range=0)
    fn=open(file,"r")
...
"""
function plot_RDF(file;ave_range=0,rmin=0.0,rmax=3.0,gmin=-1.0,gmax=-2.0)
    g=in_RDF(file,ave_range=ave_range)
    if(gmin>gmax)
...
"""
function comp_GR(file;ave_range=0)
    g=in_RDF(file,ave_range=ave_range)
    h=hr(g)
    #dr=h[2,1]-h[1,1]
    RRmax=h[end,1]
...
"""
function plot_GR(file;ave_range=0,xmin=0,xmax=4)
    GR=comp_GR(file;ave_range=ave_range)
...
"""
function plot_RGR(file;ave_range=0,xmin=0,xmax=3)
    GR=comp_GR(file;ave_range=ave_range)
...
"""
function fit_GR(file,recRmin,recRmax;ave_range=0)
    GR=comp_GR(file,ave_range=ave_range)
...
Perform fitting ``R`` vs ``R*G(R)`` by a linear function ``w[1]+R*w[2]`` in the range ``Rmin < R < Rmax``. The KB integral in the inifinite volume ``G(∞ )`` is given by the extrapolation, i.e.,  ``w[2]``.
    ...
    fit_GR(file,recRmin,recRmax;ave_range=0)
"""
function fit_RGR(file,Rmin,Rmax;ave_range=0)
    GR=comp_GR(file,ave_range=ave_range)
    x=Array{Float64}(undef,0)
    y=similar(x)
...
"""
function eval_KB(file,recRmin,recRmax;ave_range=0)
    w=fit_GR(file,recRmin,recRmax,ave_range=ave_range)
    println("G(∞ )=", w[1], " /nm^3")
    w[1]
end
...
"""
function eval_KB2(file,Rmin,Rmax;ave_range=0)
    w=fit_RGR(file,Rmin,Rmax,ave_range=ave_range)
    println("G(∞ )=", w[2], " /nm^3")
    w[2]
end
...
"""
function plot_GR_fit(file,recRmin,recRmax;ave_range=0,xmin=0,xmax=4,show_range=1)
    GR=comp_GR(file;ave_range=ave_range)
...
"""
function plot_RGR_fit(file,Rmin,Rmax;ave_range=0,xmin=0,xmax=3,show_range=1)
    GR=comp_GR(file;ave_range=ave_range)
